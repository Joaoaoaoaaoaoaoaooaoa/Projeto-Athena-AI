...
import { suggestions, type Suggestion } from "@/lib/mock-responses";
...
function Index() {
  ...
  const sendMessage = useCallback(
    async (text: string) => {
      ...
      // Create or update the conversation entry in the sidebar history.
      ...
      setMessages(history);
      historyRef.current[conversationId] = history;
      setStatus("submitted");
      ...
      try {
        const response = await fetch("/api/chat", {
          ...
          body: JSON.stringify({
            messages: history.map((m) => ({ role: m.role, content: getText(m) })),
          }),
        });

        if (!response.ok || !response.body) {
          const detail = await response.text().catch(() => "");
          throw new Error(detail || "Não consegui responder agora.");
        }
        ...
        for (;;) {
          ...
          setMessages((prev) => {
            ...
            historyRef.current[conversationId] = next;
            return next;
          });
        }
      } catch (error) {
